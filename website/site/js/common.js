...
function getReadableCoin (stats, coins, digits, withoutSymbol) {
	...
	return amount.toString() + (withoutSymbol ? '' : (' ' + stats.config.coin));
}
...
function getTransactionUrl (id, stats) {
	if (stats && blockExplorers) {
		//return blockExplorers[stats.config.coin].transactionExplorer.replace('{symbol}', stats.config.coin.toLowerCase())
		return stats.config.transactionExplorer.replace('{symbol}', stats.config.coin.toLowerCase())
			.replace('{id}', id);
	}
}
...
function getBlockchainUrl (id, stats) {
	if (stats && blockExplorers) {
		//return blockExplorers[stats.config.coin].blockchainExplorer.replace('{symbol}', stats.config.coin.toLowerCase())
		return stats.config.blockchainExplorer.replace('{symbol}', stats.config.coin.toLowerCase())
			.replace('{id}', id);
	}
}
...
function poolBlocks_ParseBlock (serializedBlock, stats) {
	...
	block = {
		...
		difficulty: parseInt(serializedBlock.difficulty),
		shares: parseInt(serializedBlock.shares),
...
function poolBlocks_RenderBlocks (blocksResults, stats) {
	...
	/* Parse mature first */
	if (matureResults) {
		for (var i = 0; i < matureResults.length; i++) {
			var block = poolBlocks_ParseBlock(matureResults[i], stats);
...
function poolBlocks_InitTemplate (ranOnce, displayedChart, xhrGetBlocks) {
	...
	updateText(`blocksTotal${coin}`, lastStats.blocks.totalBlocks.toString());
	if (lastStats.stats.poolstats.lastBlockFound) {
		var d = new Date(parseInt(lastStats.stats.poolstats.lastBlockFound * 1000))
			.toISOString();
		$(`#lastBlockFound${coin}`)
			.timeago('update', d);
	} else {
	...
	updateText(`blocksTotalSolo${coin}`, lastStats.blocks.totalBlocksSolo.toString());
	if (lastStats.stats.poolstats.lastBlockFoundSolo) {
		var d = new Date(parseInt(lastStats.stats.poolstats.lastBlockFoundSolo * 1000))
			.toISOString();
	...
	updateText(`blocksMaturityCount${coin}`, lastStats.config.unlockDepth.toString());
	...
	poolBlocks_RenderBlocks(lastStats.blocks, lastStats);
	...
	Object.keys(mergedStats)
		.forEach(key => {
			...
			poolBlocks_RenderBlocks(mergedStats[key].blocks, mergedStats[key]);
		})
...
function home_InitTemplate (parentStats, siblingStats) {
	...
	if ($(`#networkStats${coin}`)
		.length == 0) {
		minerInfo.push({
			blocks: parentStats.blocks.totalBlocks.toString(),
			blocksSolo: parentStats.blocks.totalBlocksSolo.toString(),
			coin: coin,
			symbol: parentStats.config.coin,
			miners: parentStats.miners.miners.toString(),
			minersSolo: parentStats.miners.miners.toString()
		})

		efforts.push({
			coin: coin,
			effort: `${(parentStats.stats.poolstats.roundHashes / parentStats.network.DERO.difficulty * 100).toFixed(1)}%`,
	...
	updateText(`networkHashrate${coin}`, getReadableHashRateString(parentStats.network.DERO.difficulty / parentStats.config.coinDifficultyTarget) + '/sec');
	updateText(`networkDifficulty${coin}`, formatNumber(parentStats.network.DERO.difficulty.toString(), ' '));
	updateText(`blockchainHeight${coin}`, formatNumber(parentStats.network.DERO.height.toString(), ' '));
	let rewardMinusNetworkFee = parentStats.lastblock.DERO.reward - (parentStats.lastblock.DERO.reward * (parentStats.config.networkFee ? parentStats.config.networkFee / 100 : 0))
	...
	Object.keys(siblingStats)
		.forEach(key => {
			...
			if (siblingStats[key].pool.lastBlockFound) {
				let lastChildBlockFound = parseInt(siblingStats[key].pool.lastBlockFound * 1000)
	...
	updateText('poolHashrate', `PROP: ${getReadableHashRateString(parentStats.stats.hashrate)}/sec`);
	updateText('poolHashrateSolo', `SOLO: ${getReadableHashRateString(parentStats.stats.hashrateSolo)}/sec`);


	var hashPowerSolo = parentStats.stats.hashrateSolo / (parentStats.network.DERO.difficulty / parentStats.config.coinDifficultyTarget) * 100;
	updateText('hashPowerSolo', hashPowerSolo.toFixed(2) + '%');

	var hashPower = parentStats.stats.hashrate / (parentStats.network.DERO.difficulty / parentStats.config.coinDifficultyTarget) * 100;
	updateText('hashPower', hashPower.toFixed(2) + '%');


	updateText(`poolMiners${coin}`, `${parentStats.miners.miners}/${parentStats.miners.minersSolo}`);
	updateText('blocksTotal', `${parentStats.blocks.totalBlocks}/${parentStats.blocks.totalBlocksSolo}`);


	var totalFee = parentStats.config.poolFee;
...
//		totalFee += totalDonation;
	...
	updateText(`currentEffort${coin}`, (parentStats.stats.poolstats.roundHashes / parentStats.network.DERO.difficulty * 100)
		.toFixed(1) + '%');
}
